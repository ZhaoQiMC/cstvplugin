...
public class CPUUtils {
    ...
    //预判APK运行的架构，5.0以下全默认为32位架构，5.0以上反射ApplicationInfo的abi信息，若为空则默认为当前cpu信息
    public static String checkAPKArc(Context context){
        String abi = getAppAbi(context);
        if(TextUtils.isEmpty(abi)){
            return getArchType(context);
    ...
    //反射Application的abi信息
    public static String getAppAbi(Context context){
        ...
        try {
            appInfo = context.getPackageManager()
                    .getApplicationInfo(context.getPackageName(),
                            0);
            Class<?> temp = appInfo.getClass();
            Field fa = null;
            fa = temp.getField("primaryCpuAbi");
            fa.setAccessible(true);
            abi = (String) fa.get(appInfo);
