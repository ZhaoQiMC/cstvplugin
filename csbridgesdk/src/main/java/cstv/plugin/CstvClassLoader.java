 ...
 */
class CstvClassLoader extends ClassLoader {
    //这里每个插件对应着一个ClassLoader，一旦插件更新了，则classLoader也会使用新的。
    //这样java的class就会从新的classLoader中查找，而不会去使用旧的classLoader的缓存
    private CstvPluginClassLoader[] mClassLoader = null;

    public CstvClassLoader(ClassLoader parent) {
        super(parent);
    }

    public CstvPluginClassLoader[] getClassLoaders() {
        return mClassLoader;
    }
    ...
    protected void addAPKPath(String pluginId, String dexPath, String libPath) {
        if (mClassLoader == null) {
            mClassLoader = new CstvPluginClassLoader[1];
        } else {
            int oldLenght = mClassLoader.length;
            Object[] old = mClassLoader;
            mClassLoader = new CstvPluginClassLoader[oldLenght + 1];
            arraycopy(old, 0, mClassLoader, 0, oldLenght);
        }
        mClassLoader[mClassLoader.length - 1] = new CstvPluginClassLoader(pluginId, dexPath,
    ...
    protected void removePlugin(String pluginId) {
        ...
        for (int i = 0; i < mClassLoader.length; i++) {
            CstvPluginClassLoader cl = mClassLoader[i];
            if (pluginId.equals(cl.getPluginId())) {
                ...
                Object[] old = mClassLoader;
                mClassLoader = new CstvPluginClassLoader[oldLength - 1];
                if (i != 0) {
    ...
    protected Class<?> loadClass(String className, boolean resolve) throws ClassNotFoundException {
        ...
        //挨个的到插件里进行查找
        if (mClassLoader != null) {
            for (CstvPluginClassLoader classLoader : mClassLoader) {
