 ...
 */
public class CstvPluginManager {

    private static final String PLUGIN_INSTALLED_LIST_FILE_NAME = "plugin.installedlist"; //已安装插件列表的文件地址
    private static final String PLUGIN_UPDATED_LIST_FILE_NAME = "plugin.updatedlis"; //已更新插件列表的文件地址
    ...
    private static HashMap<String, Integer> mUpdatedPluginList = null;
     ...
     * 缓存插件的对象
     */
    private static HashMap<String, CstvPlugin> mPluginMap = new HashMap<>();
    ...
    public static void init(Application application) {
        ...
        //初始化一些成员变量和加载已安装的插件
        mPackageInfo = CstvPluginUtil.getField(application.getBaseContext(), "mPackageInfo");
        ...
        Object mMainThread = CstvPluginUtil.getField(mBaseContext, "mMainThread");
        CstvPluginUtil.setField(mMainThread, "mInstrumentation", new CstvInstrumentation());
...
//        clearOldPlugin();
    ...
    public static void initAllApks(){
        installInitPlugins();
        //安装内置插件
...
//            }
//        });
//        initPluginThread.setName("initAllPluginThread");
//        initPluginThread.start();
    }

    public static void initApk(final String key){
        installInitPlugin(key);
...
//            }
//        });
//        initPluginThread.setName(key);
//        initPluginThread.start();
    }

    private static void registerComponentCallbacks(Application app){
        app.registerComponentCallbacks(new ComponentCallbacks() {
            @Override
            public void onConfigurationChanged(Configuration newConfig) {
                CstvPluginManager.onConfigurationChanged();
    ...
    private static void createPath() {
        CstvPluginUtil.createDir(CstvPluginUtil.getInsidePluginPath());
        CstvPluginUtil.createDir(CstvPluginUtil.getDexCacheParentDirectPath());
    }
    ...
    private static void installInitPlugins() {
        ...
        HashMap<String, Integer> updatedList = getUpdatedPlugin();
        for (String key : defaultList.keySet()) {
            ...
            if (updatedList.containsKey(key)) {
                updatedVersion = updatedList.get(key);
            }

            CstvPlugin plugin = getPlugin(key);
            if (defaultVersion > installVersion) {
                boolean ret = plugin.installAssetPlugin();
                //提前将dex文件优化为odex或者opt文件
                if (ret) {
                    try {
                        new DexClassLoader(CstvPluginUtil.getAPKPath(key), CstvPluginUtil.getDexCacheParentDirectPath(), null, mBaseClassLoader.getParent());
                    } catch (Throwable e) {
                        e.printStackTrace();
                    }
                }
    ...
    private static void installInitPlugin(String key) {
        HashMap<String, Integer> installedList = getInstalledPlugin();
        HashMap<String, Integer> defaultList = getDefaultPlugin();
        HashMap<String, Integer> updatedList = getUpdatedPlugin();
        ...
        boolean ret = false;
        if (defaultVersion > installVersion) {
            ret = plugin.installAssetPlugin();
        }else if(updatedVersion > installVersion){
    ...
    public static HashMap<String, Integer> getUpdatedPlugin(){
        if(mUpdatedPluginList != null) {
            return mUpdatedPluginList;
        } else {
            return mUpdatedPluginList = readUpdatedPlugin();
        }
    }
    ...
    public static HashMap<String, Integer> getDefaultPlugin() {
        return CstvPluginConfig.mDefaultList;
    }

    private static HashMap<String, Integer> readUpdatedPlugin() {
        synchronized (mInstalledPluginListLock) {
            HashMap<String, Integer> pluginList = new HashMap<>();

            String path = getPluginUpdatedListFilePath();
            ...
            if (file.exists()) {
                try {
                    ...
                    if (count > 0) {
                        ...
                        CstvPluginUtil.close(inputStream);
                        CstvPluginUtil.close(bos);
    ...
    public static boolean addUpdatedInstall(String pluginId, int version) {
        if (getUpdatedPlugin() != null){
            synchronized (mInstalledPluginListLock) {
                mUpdatedPluginList.put(pluginId, version);
            }
            saveUpdate();
            return true;
        }
    ...
    private static boolean saveInstall() {
        synchronized (mInstalledPluginListLock) {
    ...
    private static boolean saveUpdate() {
        synchronized (mInstalledPluginListLock) {
            if (mUpdatedPluginList == null) return true;
            FileOutputStream out = null;
            try {
                JSONArray array = new JSONArray();
                for (String key : mUpdatedPluginList.keySet()) {
                    int version = mUpdatedPluginList.get(key);
                    JSONObject obj = new JSONObject();
                    obj.put("id", key);
                    obj.put("version", version);
                    array.put(obj);
    ...
    private static String getInstalledPluginListFilePath() {
        return CstvPluginUtil.getInsidePluginPath() + PLUGIN_INSTALLED_LIST_FILE_NAME;
    }

    private static String getPluginUpdatedListFilePath() {
        return CstvPluginUtil.getInsidePluginPath() + PLUGIN_UPDATED_LIST_FILE_NAME;
    }
    ...
    public static boolean loadLastVersionPlugin(String pluingId) {
        CstvPlugin plugin = getPlugin(pluingId);
        CstvPluginManifest meta = plugin.getPluginMeta();
        int version = -1;
        if (meta != null) {
    ...
    public static boolean loadPlugin(String pluginId, int version) {
        synchronized (mLoadLock) {
            ...
            String pluginApkPath = CstvPluginUtil.getAPKPath(pluginId);
            CstvPlugin plugin = getPlugin(pluginId);
            if (!CstvPluginUtil.exists(pluginApkPath)) {
                if (getDefaultPlugin().containsKey(pluginId)) {
                    if (!plugin.installAssetPlugin()) {
                        return false;
                    } else {
                        pluginApkPath = CstvPluginUtil.getAPKPath(pluginId);
                    }
                } else {
                    return false;
                }
            ...
            if (CstvPluginUtil.isHotFix(pluginId)) {
                loadHotfixPluginClassLoader(pluginId);
            } else {
                //如果一个老版本的插件已经被加载了，则需要先移除
                if (getLoadedPlugin() != null && getLoadedPlugin().containsKey(pluginId)) {
                    if (cl instanceof CstvClassLoader) {
                        CstvClassLoader classLoader = (CstvClassLoader) cl;
                        //移除老版本的插件
                        classLoader.removePlugin(pluginId);
                        clearViewConstructorCache();
                        //添加新版本的插件
                        classLoader.addAPKPath(pluginId, pluginApkPath, CstvPluginUtil.getLibFileInside(pluginId));
                    }
                } else {
                    if (cl instanceof CstvClassLoader) {
                        CstvClassLoader classLoader = (CstvClassLoader) cl;
                        ...
                        CstvClassLoader classLoader = new CstvClassLoader(cl);
                        classLoader.addAPKPath(pluginId, pluginApkPath, CstvPluginUtil.getLibFileInside(pluginId));
                        CstvPluginUtil.setField(mPackageInfo, "mClassLoader", classLoader);
            ...
            if (!CstvPluginUtil.isHotfixWithoutResFile(pluginId)) {
    ...
    private static void clearCacheObject(Object drawableCache) {
        ...
        Method clearMethod = CstvPluginUtil.getMethod(drawableCache.getClass(), "clear");
        if (clearMethod != null) {
            try {
        ...
        //高android版本走这里
        Object mThemedEntries = CstvPluginUtil.getField(drawableCache, "mThemedEntries");
        if (mThemedEntries != null) {
            clearMethod = CstvPluginUtil.getMethod(mThemedEntries.getClass(), "clear");
            if (clearMethod != null) {
                try {
    ...
    private static void clearResoucesDrawableCache(Resources resouces) {
        ...
        clearCacheObject(CstvPluginUtil.getField(resouces, "mDrawableCache"));
        clearCacheObject(CstvPluginUtil.getField(resouces, "mColorDrawableCache"));
        clearCacheObject(CstvPluginUtil.getField(resouces, "mColorStateListCache"));
        clearCacheObject(CstvPluginUtil.getField(resouces, "mAnimatorCache"));
        clearCacheObject(CstvPluginUtil.getField(resouces, "mStateListAnimatorCache"));
    }
        ...
        try {
            ...
            if (mLoadedPluginList != null && mLoadedPluginList.size() != 0) {
                ...
                for (String id : mLoadedPluginList.keySet()) {
                    ...
                    if (!CstvPluginUtil.isHotfixWithoutResFile(id)) {
                        addAssetPath.invoke(assetManager, CstvPluginUtil.getAPKPath(id));
                    }
                }
            }
            //这里提前创建一个resource是因为Resources的构造函数会对AssetManager进行一些变量的初始化
            //还不能创建系统的Resources类，否则中兴系统会出现崩溃问题
            CstvPluginResources newResources = new CstvPluginResources(assetManager,
                    mBaseContext.getResources().getDisplayMetrics(),
                    mBaseContext.getResources().getConfiguration());

            CstvPluginUtil.setField(mBaseContext, "mResources", newResources);
            //这是最主要的需要替换的，如果不支持插件运行时更新，只留这一个就可以了
            CstvPluginUtil.setField(mPackageInfo, "mResources", newResources);
            ...
            CstvPluginUtil.setField(mBaseContext, "mTheme", null);
        } catch (Throwable e) {
            e.printStackTrace();
        }
    }
    ...
    private static void removeLoadedPlugin(String pluginId) {
        synchronized (mLoadLock) {
            ...
            if (mLoadedPluginList.size() == 0) {
                CstvPluginUtil.setField(mPackageInfo, "mClassLoader", mBaseClassLoader);
                Thread.currentThread().setContextClassLoader(mBaseClassLoader);
                mNowClassLoader = mBaseClassLoader;
                Thread.currentThread().setContextClassLoader(mBaseClassLoader);
            } else {
                ClassLoader cl = mNowClassLoader;
                if (!(cl instanceof CstvClassLoader)) return;
                CstvClassLoader classLoader = (CstvClassLoader) cl;
                classLoader.removePlugin(pluginId);
            }
            reloadInstalledPluginResources();
    ...
    private synchronized static void loadHotfixPluginClassLoader(String pluginId) {
        ...
        CstvHotfixClassLoader hotfixClassLoader;
        if (classLoader instanceof CstvHotfixClassLoader) {
            hotfixClassLoader = (CstvHotfixClassLoader) classLoader;
            hotfixClassLoader.addAPKPath(CstvPluginUtil.getAPKPath(pluginId),
                    CstvPluginUtil.getLibFileInside(pluginId));
        } else {
            hotfixClassLoader = new CstvHotfixClassLoader(CstvPluginUtil.getAPKPath(pluginId),
                    CstvPluginUtil.getDexCacheParentDirectPath(),
                    CstvPluginUtil.getLibFileInside(pluginId),
                    classLoader);
            hotfixClassLoader.setOrgAPKClassLoader(orgClassLoader);
            CstvPluginUtil.setField(orgClassLoader, "parent", hotfixClassLoader);
        }
    ...
    private static void loadInstalledPlugins() {
        synchronized (mLoadLock) {
            ...
            CstvClassLoader classLoader = null;
            ...
            for (String pluginId : installedPluginMaps.keySet()) {
                if (CstvPluginUtil.isPlugin(pluginId)) {
                    if (classLoader == null) {
                        classLoader = new CstvClassLoader(mBaseContext.getClassLoader());
                    }
                    classLoader.addAPKPath(pluginId, CstvPluginUtil.getAPKPath(pluginId), CstvPluginUtil.getLibFileInside(pluginId));
                    putLoadedPlugin(pluginId, installedPluginMaps.get(pluginId));
                    isNeedLoadResource = true;
                }
                //热修复补丁,补丁一般只针对某个版本
                if (CstvPluginUtil.isHotFix(pluginId)) {
                    CstvPluginManifest manifest = getPlugin(pluginId).getPluginMeta();
                    try {
     ...
     * @return
     */
    public static CstvPlugin getPlugin(String pluginId) {
        CstvPlugin plugin = null;
        try {
            plugin = mPluginMap.get(pluginId);
            if (plugin != null) {
                return plugin;
            }
            if (CstvPluginUtil.iszeusPlugin(pluginId)) {
                plugin = new CstvPlugin(pluginId);
            }
    ...
    public static String getInsideLibPath(String pluginId) {
        return CstvPluginUtil.getLibFileInside(pluginId);
    }

    public static void startActivity(Activity activity, Intent intent) {
        ComponentName componentName = intent.getComponent();
        intent.setClassName(componentName.getPackageName(), CstvPluginConfig.PLUGIN_ACTIVITY_FOR_STANDARD);
        intent.putExtra(CstvPluginConfig.PLUGIN_REAL_ACTIVITY, componentName.getClassName());
